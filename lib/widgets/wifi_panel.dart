...
class _WifiPanelState extends State<WifiPanel> {
  ...
  void initState() {
    ...
    _ipController.text = svc.arduinoIp;
    svc.fetchCats();
    svc.startPolling();
  }
  ...
  Future<void> _startScan() async {
    setState(() { _isScanning = true; _scannedUid = null; });
    final uid = await context.read<ArduinoService>().scanRfid();
    setState(() { _isScanning = false; _scannedUid = uid; });
    _showSnack(uid != null ? 'UID 감지: $uid' : '스캔 실패 또는 시간 초과');
  }
  ...
  Future<void> _register() async {
    ...
    final ok = await context.read<ArduinoService>()
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Text(svc.connected ? '아두이노 연결됨' : '연결 안 됨',
        ...
        Row(children: [
          Expanded(child: _ValueCard(
            ...
            value: svc.status.catDetected
                ? '${(svc.status.weight / 1000).toStringAsFixed(2)} kg' : '--',
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: svc.status.catDetected
                ? Colors.green.withOpacity(0.1) : cs.surfaceContainerLow,
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
                color: svc.status.catDetected ? Colors.green : cs.outlineVariant.withOpacity(0.5)),
          ),
          child: Row(children: [
            Text(svc.status.catDetected ? '🐱' : '😴',
                style: const TextStyle(fontSize: 28)),
            const SizedBox(width: 12),
            Text(
              svc.status.catDetected
                  ? '${svc.status.catName} 감지됨!' : '고양이 없음',
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(_isScanning ? '태그를 갖다 대세요...' : 'RFID 스캔 시작'),
        ...
        if (_scannedUid != null) ...[
          ...
          Container(
            ...
            child: Text('감지된 UID: $_scannedUid',
        ...
        svc.cats.isEmpty
          ...
          child: Center(child: Text('등록된 고양이가 없어요',
              style: TextStyle(color: cs.onSurfaceVariant))),
        )
            : Column(
          children: svc.cats.map((cat) => Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(
                ...
                children: [
                  Text(cat.name, style: tt.bodyMedium?.copyWith(
                      fontWeight: FontWeight.w700)),
                  Text('UID: ${cat.uid}',
...
class _ValueCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: highlight ? Colors.orange.withOpacity(0.1) : cs.surfaceContainerLow,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
